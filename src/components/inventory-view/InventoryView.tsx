...
import { ItemCreatorFactoryByName } from '../../logic/items/item-creator-factory/index';
...
const InventoryView: FC<InventoryViewProps> = ({currentDropName, inventoryClass}) => {
    
    const [inventory, setInventory] = useState(inventoryClass);
    ...
    const dropHandler = (e: React.DragEvent<HTMLImageElement>) => {
        ...
        console.log(currentDropName);
        let item = ItemCreatorFactoryByName(currentDropName);

        if (item !== null) {

            inventory.addItem(item);
            setInventoryItems(inventoryItems => [...inventory.items])
        }
